...
*   **Declarative Presets & Single Source of Truth**:
    ...
    *   **Parity Verification**: Run `make check` (or `go test -v ./internal/preset`) to verify that `README.md` matches `data/presets.json` and complies with all path invariants.
...
*   `internal/preset/`: Embedded presets catalog loader and README table synchronizer.
